...
class MyGameCharacter {
     ...
     * @param {number} [eID=1] 敵の種類。
     * @param {number} [hpRec=1] 撃破字回復量。
     * @param {number} [speed=1] 素早さ。
     */
    constructor(name, imageURL = null, hp, eID, hpRec, speed = 1) {
        this.name = name;
        this.hp = hp;
        this.eID = eID;
        this.hpRec = hpRec;
    ...
    attackTo(targetCharacter, wepon) {
        ...
        if (wepon.checkCount()) {
            for (let i = 0; i < wepon.attackCount; i++) {
                let attack = wepon.calcAttackBonus() + this.eID;
                if (this.checkHit(targetCharacter)) {
                    hitCount++;
                    damage += Math.max(0, attack - targetCharacter.hpRec);
                }
            }
            wepon.use();
    ...
    print(htmlElement) {
        ...
        htmlElement.innerHTML += ("腕力:" + this.eID + "<br />");
        htmlElement.innerHTML += ("防御:" + this.hpRec + "<br />");
